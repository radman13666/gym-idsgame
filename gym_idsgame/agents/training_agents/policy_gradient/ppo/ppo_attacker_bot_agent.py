...
class PPOAttackerBotAgent(BotAgent):
    """
    ...
    def __init__(self, pg_config: PolicyGradientAgentConfig, game_config: GameConfig, model_path: str = None,
        ...
        """
        super(PPOAttackerBotAgent, self).__init__(game_config)
    ...
    def initialize_models(self) -> None:
        ...
        # Initialize models
        self.attacker_policy_network = FFNActorCritic(
        ...
        self.attacker_policy_network.to(self.device)
    ...
    def action(self, game_state: GameState) -> int:
        ...
        attacker_action = self.get_action(attacker_state)

        # convert to global action
        global_action = PolicyGradientAgent.convert_local_attacker_action_to_global(attacker_action, attacker_obs)
    ...
    def get_action(self, state: np.ndarray) -> int:
        ...
        legal_actions = list(filter(lambda action: self.idsgame_env.is_attack_legal(action), actions))
        non_legal_actions = list(filter(lambda action: not self.idsgame_env.is_attack_legal(action), actions))
